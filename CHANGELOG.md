...
- The highlight-server protocol is now versioned and capability-advertising: the ready line carries `protocol:2` and a `modes` array listing exactly the request modes with live handlers (`raw`, `diff-wash`, `unified-diff`). A client can gate on the array and never wastes a sacrificial request probing an unimplemented mode. Extra ready-line fields are additive, so v1 clients that only check `ready:1` keep working.
- `mode:"diff-wash"` applies a per-line background wash to raw highlighting. The request takes `wash_per_line`: an array with one entry per logical line of `code`, each `null` (no wash) or a `#rrggbb` background. The wash is re-asserted after every `\x1b[0m` reset syntect emits (the Phase 1 primitive) and cleared before the line ending, so it never leaks across lines. Length mismatch, invalid hex, or an unknown mode fail closed with the request id echoed.
- `mode:"unified-diff"` highlights a unified diff body with mixed language+diff tokens. Deleted (`-`) and added (`+`) lines each run through an independent lexical stream, so an open comment on one side cannot poison the other; the `-`/`+` marker is painted in the delete/insert color while the content gets normal language token colors (`diffStyle:"marker-fg"`, the default). Structural lines (`diff --git`, `---`/`+++` file headers, `@@` hunk ranges, `\ No newline at end of file`, context lines) render verbatim with no marker color. `diffStyle:"bg-wash"` additionally paints a soft per-line background derived by blending the marker color ~22% toward black (so the full-payload wash is obvious but not loud, keeping syntax foreground readable), while the marker keeps its bright color. `colors` optionally overrides the inserted/deleted hex (defaults `#78e38c` / `#ff5d7a`). The payload is always byte-identical after ANSI stripping, line endings (`\n` vs `\r\n`) and trailing-newline shape are preserved, and no host frame is ever synthesized. Already-ANSI input fails closed rather than being double-encoded. New `diff` module with the dual-stream engine; integration fixtures in `tests/unified_diff_server.rs` drive the real binary end to end.
- `highlight::LineHighlighter` (crate-internal) provides a persistent per-stream highlighter so dual-stream diff composition keeps independent lexical state.
